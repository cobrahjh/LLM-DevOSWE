...
c.on('open', async () => {
    ...
    if (r.result && r.result.data) {
        ...
        const path = 'C:/Users/Stone-PC/Desktop/ai-autopilot-cruise.png';
        fs.writeFileSync(path, buf);
        console.log('Screenshot saved to', path, '(' + buf.length + ' bytes)');
    } else {
        console.log('Screenshot failed:', JSON.stringify(r));
    }
    c.close();
    process.exit(0);
});
