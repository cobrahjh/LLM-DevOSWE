/**
 * METAR glass - SimGlass v2.0.0
 * Real-time aviation weather from aviationweather.gov
 */

class MetarGlass extends SimGlassBase {
    constructor() {
        super({
            widgetName: 'metar-glass',
            widgetVersion: '2.0.0',
    ...
    loadState() {
        try {
            const saved = localStorage.getItem('metar-glass-state');
            if (saved) {
    ...
    saveState() {
        try {
            localStorage.setItem('metar-glass-state', JSON.stringify({
    ...
    showLoading() {
        ...
        text.style.color = 'var(--glass-text-muted)';
...
document.addEventListener('DOMContentLoaded', () => {
    window.metarGlass = new MetarGlass();
    window.addEventListener('beforeunload', () => window.metarGlass?.destroy());
});
