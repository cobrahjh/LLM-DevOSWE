/**
 * Map glass - SimGlass v2.1.0
 * Live aircraft position map using Leaflet
 *
 * glass Interconnection:
 * - Receives waypoint-select from flightplan-glass
 * - Receives route-update from flightplan-glass
 * - Receives vatsim-traffic from vatsim-live glass
 * - Broadcasts position-update to other widgets
 */

class MapGlass extends SimGlassBase {
    constructor() {
        super({
            widgetName: 'map-glass',
            widgetVersion: '2.1.0',
            autoConnect: true
        });
    ...
    initSyncListener() {
        this.syncChannel.onmessage = (event) => {
            ...
            switch (type) {
                ...
                case 'vatsim-traffic':
                    // VATSIM traffic data from vatsim-live glass
...
document.addEventListener('DOMContentLoaded', () => {
    window.mapGlass = new MapGlass();
    window.addEventListener('beforeunload', () => window.mapGlass?.destroy());
});
