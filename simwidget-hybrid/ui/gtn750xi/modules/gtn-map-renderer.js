...
class GTNMapRenderer {
    ...
    renderMap() {
        ...
        if (state.systemSettings?.runwayExtensions && state.destinationRunways) {
            this.renderRunwayExtensions(ctx, w / 2, h / 2, w, h, state);
        }
    ...
    renderObsCourseLine(ctx, cx, cy, w, h, state) {
        ...
        if (state.obs.holdingPattern) {
            this.renderObsHoldingPattern(ctx, wpPos, w, h, state);
        }

        ctx.restore();
    }

    renderObsHoldingPattern(ctx, wpPos, w, h, state) {
