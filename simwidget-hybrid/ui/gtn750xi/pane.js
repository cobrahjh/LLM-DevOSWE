/**
 * GTN750Xi GPS Glass v1.0.0 - Modernized GTN750 with Enhanced Features
...
class GTN750XiPane extends SimGlassBase {
    constructor() {
        super({
            widgetName: 'gtn750xi',
            widgetVersion: '1.0.0',
    ...
    _checkNorthUpAbove() {
        ...
        if (this.map.range >= threshold && this.map.orientation !== 'north') {
            ...
            GTNCore.log(`[GTN750Xi] North Up Above: Switched to North Up at ${this.map.range}nm`);
            ...
            GTNCore.log(`[GTN750Xi] North Up Above: Restored ${this.map.orientation} orientation at ${this.map.range}nm`);
        }
    }

    /**
     * Fetch runway data for destination airport (for runway extensions)
     */
    async _updateDestinationRunways() {
        ...
        try {
            ...
            GTNCore.log(`[GTN750Xi] Loaded ${this.destinationRunways.length} runways for ${destination.icao}`);
        } catch (e) {
            GTNCore.log(`[GTN750Xi] Failed to fetch runways for ${destination.icao}: ${e.message}`, 'WARN');
            this.destinationRunways = null;
        }
    }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        window.gtn750xi = new GTN750XiPane();
        handleUrlHash();
        window.addEventListener('beforeunload', window.gtn750xi._beforeUnloadHandler);
    });
} else {
    window.gtn750xi = new GTN750XiPane();
    handleUrlHash();
