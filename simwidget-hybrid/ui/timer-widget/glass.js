/**
 * Timer glass - SimGlass v2.0.0
 * Stopwatch and countdown timer for flight procedures
 */

class TimerGlass extends SimGlassBase {
    constructor() {
        super({
            widgetName: 'timer-glass',
            widgetVersion: '2.0.0',
    ...
    saveState() {
        try {
            localStorage.setItem('timer-glass-state', JSON.stringify({
                mode: this.mode,
    ...
    loadState() {
        try {
            const saved = localStorage.getItem('timer-glass-state');
            if (saved) {
...
document.addEventListener('DOMContentLoaded', () => {
    window.timerGlass = new TimerGlass();
    window.addEventListener('beforeunload', () => window.timerGlass?.destroy());
});
