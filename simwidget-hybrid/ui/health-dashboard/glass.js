/**
 * Health Dashboard Glass v2.0.0
 * Path: simwidget-hybrid/ui/health-dashboard/glass.js
 *
 * Real-time system health monitoring for SimGlass components
 */

class HealthDashboardGlass extends SimGlassBase {
    constructor() {
        super({
        ...
        this.WIDGETS = [
            { id: 'copilot', name: 'Copilot', icon: '🧑‍✈️', path: '/ui/copilot-glass/' },
            { id: 'checklist', name: 'Checklist', icon: '✅', path: '/ui/checklist-glass/' },
            { id: 'flightplan', name: 'FPL', icon: '🛫', path: '/ui/flightplan-glass/' },
            { id: 'map', name: 'Map', icon: '🗺️', path: '/ui/map-glass/' },
            { id: 'weather', name: 'Weather', icon: '🌤️', path: '/ui/weather-glass/' },
            { id: 'timer', name: 'Timer', icon: '⏱️', path: '/ui/timer-glass/' },
            { id: 'simbrief', name: 'SimBrief', icon: '📋', path: '/ui/simbrief-glass/' },
            { id: 'notepad', name: 'Notes', icon: '📝', path: '/ui/notepad-glass/' },
            { id: 'camera', name: 'Camera', icon: '📷', path: '/ui/camera-control/' },
    ...
    async checkWidgetHealth() {
        for (const glass of this.WIDGETS) {
            try {
                ...
                const el = document.getElementById('glass-' + glass.id);
                if (el) {
                    el.className = 'glass-status ' + (response.ok ? 'ok' : 'error');
                }
            } catch (err) {
                const el = document.getElementById('glass-' + glass.id);
                if (el) {
                    el.className = 'glass-status error';
                }
            }
        }
    }
    ...
    createWidgetElement(glass) {
        const div = document.createElement('div');
        div.className = 'glass-status unknown';
        div.id = 'glass-' + glass.id;
        ...
        const name = document.createElement('span');
        name.className = 'glass-name';
        name.textContent = glass.name;

        const dot = document.createElement('span');
        dot.className = 'glass-dot';
...
document.addEventListener('DOMContentLoaded', () => {
    window.healthDashboard = new HealthDashboardGlass();
