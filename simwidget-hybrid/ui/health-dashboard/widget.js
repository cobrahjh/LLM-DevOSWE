...
class HealthDashboardWidget extends SimGlassBase {
    ...
    async checkWidgetHealth() {
        for (const widget of this.WIDGETS) {
            try {
                const response = await fetch(this.API_URL + widget.path, { method: 'HEAD' });
    ...
    initWidgetsGrid() {
        ...
        this.WIDGETS.forEach(widget => {
            this.elements.widgetsGrid.appendChild(this.createWidgetElement(widget));
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.healthDashboard = new HealthDashboardWidget();
