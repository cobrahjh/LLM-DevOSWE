...
class WeatherGlass extends SimGlassBase {
    constructor() {
        super({
            widgetName: 'weather-glass',
            widgetVersion: '2.0.0',
...
Category: ${category}`;
    ...
    saveState() {
        try {
            localStorage.setItem('weather-glass-state', JSON.stringify({
    ...
    loadState() {
        try {
            const saved = localStorage.getItem('weather-glass-state');
            if (saved) {
...
document.addEventListener('DOMContentLoaded', () => {
    window.weatherGlass = new WeatherGlass();
    window.addEventListener('beforeunload', () => window.weatherGlass?.destroy());
});
