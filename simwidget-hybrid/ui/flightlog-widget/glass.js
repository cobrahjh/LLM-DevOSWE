/**
 * Flight Log glass - SimGlass
 * Auto-logs flights with stats and totals
 */

class FlightLogGlass extends SimGlassBase {
    constructor() {
        super({
    ...
    saveFlights() {
        try {
            localStorage.setItem('flightlog-glass-flights', JSON.stringify(this.flights));
        } catch (e) {
            if (window.telemetry) {
                telemetry.captureError(e, {
                    operation: 'saveFlights',
                    glass: 'flightlog-glass',
    ...
    loadFlights() {
        try {
            const saved = localStorage.getItem('flightlog-glass-flights');
            if (saved) {
...
document.addEventListener('DOMContentLoaded', () => {
    window.flightLogGlass = new FlightLogGlass();
    window.addEventListener('beforeunload', () => window.flightLogGlass?.destroy());
});
