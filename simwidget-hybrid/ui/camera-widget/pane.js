...
class CameraPane extends SimGlassBase {
    constructor() {
        super({
    ...
    initPlatformIndicator() {
        ...
        platformSection.innerHTML = `
            ...
            <div class="platform-indicator" id="platformIndicator">
                ...
                <button class="platform-refresh" onclick="window.CameraPane.refreshPlatform()" title="Refresh platform detection">🔄</button>
            </div>
    ...
    async sendCommand(action, body = {}) {
        try {
            // Map pane actions to backend commands
            const actionMap = {
                'toggle': 'VIEW_MODE',
...
document.addEventListener('DOMContentLoaded', () => {
    window.CameraPane = new CameraPane();
    window.addEventListener('beforeunload', () => window.CameraPane?.destroy());
});
