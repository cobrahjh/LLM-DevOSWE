...
class VatsimLivePane extends SimGlassBase {
    constructor() {
        super({
    ...
    broadcastToMap() {
        ...
        // Send nearby aircraft to Map pane
        const nearby = this.pilots.filter(pilot => {
            const distance = this.calculateDistance(pilot.latitude, pilot.longitude);
...
document.addEventListener('DOMContentLoaded', () => {
    window.VatsimLivePane = new VatsimLivePane();
    window.addEventListener('beforeunload', () =>
        window.VatsimLivePane?.destroy()
    );
});
