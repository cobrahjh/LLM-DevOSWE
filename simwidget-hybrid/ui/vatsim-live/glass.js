...
class VatsimLiveGlass extends SimGlassBase {
    constructor() {
        super({
    ...
    broadcastToMap() {
        ...
        // Send nearby aircraft to map glass
        const nearby = this.pilots.filter(pilot => {
            const distance = this.calculateDistance(pilot.latitude, pilot.longitude);
    ...
    showNotification(title, message, type = 'info') {
        ...
        // Also show in-glass notification
...
document.addEventListener('DOMContentLoaded', () => {
    window.vatsimLiveGlass = new VatsimLiveGlass();
    window.addEventListener('beforeunload', () =>
        window.vatsimLiveGlass?.destroy()
    );
});
