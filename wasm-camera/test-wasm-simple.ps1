# WASM Camera Test Script - Simple Version
...
if ($msfs) {
    ...
    Write-Host "  ERROR - MSFS is not running. Please start MSFS first." -ForegroundColor Red
    exit 1
}
...
if (Test-Path $CommunityPath) {
    Write-Host "  OK - Found: $CommunityPath" -ForegroundColor Green
} else {
    ...
    if (Test-Path $altPath) {
        ...
        Write-Host "  ERROR - Cannot find Community folder" -ForegroundColor Red
...
$sourcePath = "C:\Temp\package\simwidget-camera"
$targetPath = Join-Path $CommunityPath "simwidget-camera"

if (Test-Path $sourcePath) {
    if (Test-Path $targetPath) {
        Remove-Item $targetPath -Recurse -Force
        Write-Host "  INFO - Removed existing installation" -ForegroundColor Cyan
    }
    Copy-Item $sourcePath $targetPath -Recurse
    Write-Host "  OK - WASM package installed to Community folder" -ForegroundColor Green
    Write-Host "    Note: Restart MSFS or reload scenery for changes to take effect" -ForegroundColor Yellow
} else {
    Write-Host "  ERROR - Source package not found at: $sourcePath" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "  OK - Lorby AAO is running on port 43380" -ForegroundColor Green
} catch {
    Write-Host "  WARNING - Lorby AAO not responding on port 43380" -ForegroundColor Red
...
try {
    ...
    Write-Host "  OK - SimWidget server is running on port 8080" -ForegroundColor Green
} catch {
    Write-Host "  WARNING - SimWidget server not responding on port 8080" -ForegroundColor Red
...
try {
    ...
    Write-Host "  OK - WASM camera endpoint responded" -ForegroundColor Green
    Write-Host "    Status: $($statusData | ConvertTo-Json -Depth 3)" -ForegroundColor Cyan
} catch {
    Write-Host "  WARNING - WASM camera endpoint not available yet" -ForegroundColor Yellow
...
try {
    ...
    $response = Invoke-WebRequest -Uri "http://localhost:8080/api/wasm-camera" -Method POST -Body $testCmd -ContentType "application/json" -TimeoutSec 5 -UseBasicParsing -ErrorAction Stop
    Write-Host "  OK - Command sent successfully" -ForegroundColor Green
    Write-Host "    Response: $($response.Content)" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  Check MSFS for camera movement!" -ForegroundColor Yellow
} catch {
    Write-Host "  WARNING - Could not send test command" -ForegroundColor Yellow
    Write-Host "    Error: $($_.Exception.Message)" -ForegroundColor Gray
}
