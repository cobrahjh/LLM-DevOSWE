#!/bin/bash
# Claude Code Statusline - Session context, cost, and 5H usage
# Adapted for Windows (Git Bash) from fcakyon/claude-codex-settings
...
if [ -f "$CACHE_FILE" ]; then
    if [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "mingw"* ]]; then
        FILE_TIME=$(stat -c %Y "$CACHE_FILE" 2>/dev/null || date -r "$CACHE_FILE" +%s 2>/dev/null || echo 0)
...
if [ -z "$API" ]; then
    ...
    # Windows/Linux: check credentials file
    if [ -z "$CREDS" ]; then
        CRED_FILE="$HOME/.claude/.credentials.json"
        [ -f "$CRED_FILE" ] && CREDS=$(cat "$CRED_FILE")
    fi
    ...
    if [ -n "$TOKEN" ]; then
        ...
        [ -n "$API" ] && echo "$API" > "$CACHE_FILE" 2>/dev/null
    fi
fi
