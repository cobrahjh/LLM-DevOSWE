...
function sendAlert(severity, source, title, message, service) {
    const cooldownKey = `${source}:${service || 'global'}`;
...
async function sendSlackAlert(severity, title, message, service) {
    ...
    const icon = severity === 'critical' ? ':rotating_light:' : severity === 'error' ? ':red_circle:' : severity === 'warning' ? ':warning:' : ':large_blue_circle:';
...
app.get('/api/alerts', (req, res) => {
    const { severity, limit: lim, since, unacked } = req.query;
    ...
    if (unacked === 'true') { conditions.push('acknowledged = 0'); }
